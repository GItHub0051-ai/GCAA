...
from networks.subnet.CAmodule import CAmodule
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, visual=None, acoustic=None, visual_ids=None, acoustic_ids=None, pos_tag_ids=None, 
        ...
        ##
        sequence_output = self.CAmodule(sequence_output)
        ##
        # torch.save(sequence_output, 'CE_entropy')
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, visual=None, visual_ids=None, **kwargs):
        x = features[:, 0, :]  # take <s> token (equiv. to [CLS]) [32, 768]
        x = self.dropout(x)
        x = self.dense(x.to(device))
        x = torch.selu(x)
        x = self.dropout(x)
        x = self.out_proj(x)
        return x
